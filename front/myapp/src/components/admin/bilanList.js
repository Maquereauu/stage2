    ...
    return <>{newlist.map((Bilan,key)=>{
        return <div key={key} className="box2 margin-top">
            ...
            <div className="background-color-2-3">
                <div className="margin-bottom--- flex space-evenly">
                    {("image" in Bilan)?<img className="prod-img"src={"./img/"+Bilan.image}></img>:<p>{Bilan.text}</p>}
            </div>
            </div>
            </div>})}
        <div className='flex2 space-around'>
        <Button variant="secondary" disabled={disabled} onClick={()=>{props.handleShowModalBilanInsert();setDisabled(true);}}>Insérer des infos sur un bilan</Button>
        </div>
        </>
}
